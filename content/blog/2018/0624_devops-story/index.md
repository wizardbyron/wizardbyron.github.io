...
在 2009 年第一届 DevOpsDays 上，《敏捷教练》的作者 Rachel Davies 作为第一届 DevOpsDays 上的第一位分享嘉宾。分享了在 BBC 采用用户故事跟踪非功能需求的经验。然而这一实践并不如 DevOps 的其他实践那样广泛。这个实践实际上很简单，就是把非功能需求做为用户故事的 AC 放入故事卡里。
...
通过以上三个 DevOps 故事，我们可以看出：**完成一个 DevOps 史诗故事是需要不同角色的共同参与的，而且每个角色都能通过自己的 DevOps 故事让其他团队成员获益**。此外，在以上三个 DevOps 故事中，都包含了 DevOps 故事的几个原则：
